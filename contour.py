...
def masqueSansQueue(segmentation):
    ...
    _, contours, hierarchy = cv2.findContours(segmentation, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    _ , contours, hierarchy = cv2.findContours(masquethresh, 1, cv2.CHAIN_APPROX_NONE)
    contourUtile = max(contours, key=len)
...
#methode cercle :
    ...
    #On parcours la moitie des points et 1 point sur 10 (d'ou le /20 et le i*10)
...
def checkCercle(contourUtile, centre, input, debug = False):
    ...
    for i in range(int(len(contourUtile)/20.0)):
        point = contourUtile[10*i][0]
        if debug == True :
        ...
        if (int(i/(len(contourUtile)/20.0)*100)%10 ==0):
            print ("{} %  ".format(int(i/(len(contourUtile)/20.0)*100)),)
    print ("")
...
def analyse(input):
    ...
    ellipse, epaisseur = checkEllipse(grandAxe1, grandAxe2, contourUtileMasque, input)
